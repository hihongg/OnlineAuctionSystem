...
Dưới đây là sơ đồ cấu trúc các lớp cốt lõi hiện tại của hệ thống:

```mermaid
classDiagram
    DatabaseConnection "1" --o "1" UserDAO : sử dụng
    UserDAO ..> User : quản lý
    ClientHandler "1" -- "1" UserDAO : gọi hàm
    ClientHandler ..|> Runnable : triển khai
    Server "1" *-- "many" ClientHandler : tạo ra
    ...
    class UserDAO {
        -connection: Connection
        +UserDAO()
        +registerUser(username, password, email, role) boolean
